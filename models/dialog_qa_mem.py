...
from models.memory_encoders import MemoryEncoder
...
def get_final_encoder_states(encoder_outputs: torch.Tensor,
    ...
    """
        Adapted from allennlp.nn.util.get_final_encoder_states
        to avoid error in encoder_outputs.gather
    """
    last_word_indices = mask.sum(1).long() - 1
    last_word_indices[last_word_indices < 0] = 0 # if last_word_indices < 0, it was a blank question,
    # to avoid an error, we change this value to 0
...
class DialogQA(Model):
    ...
    def __init__(self, vocab: Vocabulary,
                 ...
                 memory_encoder: MemoryEncoder, # this is used to get some m_t
                 span_start_encoder: Seq2SeqEncoder,
                 span_end_encoder: Seq2SeqEncoder,
        ...
        # self._memory_question_encoder = nn.Linear(self._encoding_dim * 2, self._encoding_dim)
        max_turn_length = 12

        self._matrix_attention = LinearMatrixAttention(self._encoding_dim, self._encoding_dim, 'x,y,x*y')
        self._merge_atten = TimeDistributed(torch.nn.Linear(self._encoding_dim * 5, self._encoding_dim))
    ...
    def forward(self,  # type: ignore
        ...
        _, _, max_a_len, _ = answer['token_characters'].size()
        ...
        embedded_answer = embedded_answer.reshape(total_qa_count, max_a_len,
                                                      self._text_field_embedder.get_output_dim())
        ...
        encoded_answer = self._variational_dropout(self._phrase_layer(embedded_answer, answer_mask))
        ...
        encoded_question_h = get_final_encoder_states(encoded_question, question_mask, True).view(
            batch_size, max_qa_count, -1
        )
        encoded_answer_h = get_final_encoder_states(encoded_answer, answer_mask, True).view(
            batch_size, max_qa_count, -1
        )

        # print("repeated_encoded_passage", repeated_encoded_passage.size())
        # print("encoded_question", encoded_question.size())
        m_t = self._memory_encoder(encoded_question_h, encoded_answer_h, qa_mask.view(batch_size, max_qa_count))
        m_t_repeated = m_t.view(batch_size * max_qa_count, 1, self._encoding_dim).expand(batch_size * max_qa_count, passage_length, self._encoding_dim)

        # Shape: (batch_size * max_qa_count, passage_length, encoding_dim * 5)
