...
def make_reading_comprehension_instance_quac(question_list_tokens: List[List[Token]],
    ...
    def answer_tokens(passage_tokens, span_start, span_end):
        return passage_tokens[span_start:span_end]

    last_ann = [t[-1] for t in token_span_lists]
    ...
    metadata = {'original_passage': passage_text,
                ...
                'passage_tokens': [token.text for token in passage_tokens], }
    p_answer_marker_list: List[Field] = []
    ...
    def mark_tag(span_start, span_end, passage_tags, prev_answer_distance):
        ...
        if span_start == span_end:
            passage_tags[span_start] = get_tag(prev_answer_distance, "")
        else:
            passage_tags[span_start] = get_tag(prev_answer_distance, "start")
            passage_tags[span_end] = get_tag(prev_answer_distance, "end")
            for passage_index in range(span_start + 1, span_end):
                passage_tags[passage_index] = get_tag(prev_answer_distance, "in")

    if token_span_lists:
        span_start_list: List[Field] = []
        span_end_list: List[Field] = []
        p_starts, p_ends = deque(maxlen=num_context_answers), deque(maxlen=num_context_answers)
        ...
        for question_index, answer_span_lists in enumerate(token_span_lists):
            ...
            if num_context_answers > 0:
                for i in range(len(p_starts)):
                    mark_tag(p_starts[i], p_ends[i], prev_answer_marker_lists, i + 1)
            p_starts.append(span_start)
            p_ends.append(span_end)
            if num_context_answers > 0:
                p_answer_marker_list.append(SequenceLabelField(prev_answer_marker_lists,
                                                                passage_field,
